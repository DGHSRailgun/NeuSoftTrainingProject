...
//        System.out.println(result.data.toString());
    ...
    public BaseModel addManInfo(@Validated({InsertGroup.class}) @RequestBody ManManufacturer manManufacturer, BindingResult bindingResult){
        if(bindingResult.hasErrors()){
            throw BusinessException.INSERT_FAIL.newInstance(this.getErrorResponse(bindingResult),
                    new Object[] {manManufacturer.toString()});
        }else {
            BaseModel result = new BaseModel();
            int i = manManufacturerService.insert(manManufacturer);
            if (i == 1) {
                result.code = 200;
                return result;
            } else {
